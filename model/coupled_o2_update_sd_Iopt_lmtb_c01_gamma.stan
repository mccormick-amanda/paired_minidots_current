...
parameters {
  ...
  real<lower=1> gamma_1[2]; // scaling of gpp with temperature
  real<lower=1> gamma_2[2]; // scaling of er with temperature
...
transformed parameters {
  ...
  for (n in 1:n_obs) {
    ...
    for (j in 1:2) {
      // gpp
      b[n,j] = b0[map_days[n],j]*gamma_1[j]^(temp[n,j] - temp_ref);
      ...
      // er
      kappa[n,j] = r[map_days[n],j]*gamma_2[j]^(temp[n,j] - temp_ref);
      // nep
...
model {
  ...
  // D ~ normal(0, 1) T[0, ];
  c0 ~ normal(0, 1); //was -3, 3
  c1 ~ normal(0, 1); //was -3, 3
  for (j in 1:2) { 
    //a[j] ~ normal(0, 1) T[0, ];
    opt[j] ~ normal(0, 1) T[0, ];
    gamma_1[j] ~ normal(1, 0.05) T[1, ]; 
    gamma_2[j] ~ normal(1, 0.05) T[1, ];    
  }
